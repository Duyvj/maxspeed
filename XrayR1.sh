...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/Duyvj/maxspeed/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
uninstall() {
    confirm "Bạn có chắc muốn gỡ cài đặt XrayR không? ( Mặc định No )"
    if [[ $? != 0 ]]; then
        if [[ $# == 0 ]]; then
            show_menu
        fi
...
--- https://github.com/XrayR-project/XrayR ---
  ${green}0.${plain} Mở file Config XrayR
————————————————
